.PHONY: tests help install venv lint isort tcheck build commit-checks prepare pypibuild pypipush docker update-all-dockerhub-readmes
SHELL := /usr/bin/bash
.ONESHELL:
...
tcheck: venv
	@$(venv_activated)
	# mypy openclaw_k8s_toggle_operator
	mypy .
...
PKG_SOURCES := openclaw_k8s_toggle_operator/*
...
dist/openclaw_k8s_toggle_operator-$(VERSION).tar.gz dist/openclaw_k8s_toggle_operator-$(VERSION)-py3-none-any.whl dist/.touchfile: $(PKG_SOURCES) $(VENV_DEPS) pyproject.toml
...
pypibuild: venv dist/openclaw_k8s_toggle_operator-$(VERSION).tar.gz dist/openclaw_k8s_toggle_operator-$(VERSION)-py3-none-any.whl

dist/.touchfile_push: dist/openclaw_k8s_toggle_operator-$(VERSION).tar.gz dist/openclaw_k8s_toggle_operator-$(VERSION)-py3-none-any.whl
	@$(venv_activated)
...
DOCKER_IMAGE := openclaw-k8s-toggle-operator
...
update-all-dockerhub-readmes:
	...
	TOKEN=$$(curl -s -X POST https://hub.docker.com/v2/users/login/ \
	  ...
	  | jq -r .token) && \
	for mapping in \
	  ".:xomoxcc/openclaw-k8s-toggle-operator"; do \
